...
class APIClient {
	...
	get = async <ReturnType>(url: string, config?: AxiosRequestConfig) => {
		try {
			const res = await this.axios_instance.get<ResponseData<ReturnType>>(`${this.endpoint}${url}`, config);
	 ...
	 * @updated By: Alfonso
	 */
	post = async <ReturnType, T = unknown>(url?: string, params?: T, config?: AxiosRequestConfig) => {
		try {
			const res = await this.axios_instance.post<ResponseData<ReturnType>>(`${this.endpoint}${url}`, params, config);
			throwAuthenticationError(res.data.error);
			return res.data;
		} 
		catch (error) {
	 ...
	 * @updated By: Alfonso
	 */
	put = async <ReturnType, T = unknown>(url?: string, params?: T, config?: AxiosRequestConfig) => {
		try {
			const res = await this.axios_instance.put<ResponseData<ReturnType>>(`${this.endpoint}${url}`, params, config);
			throwAuthenticationError(res.data.error);
			return res.data;
		} 
		catch (error) {
	...
	delete = async <ReturnType>(url?: string, config?: AxiosRequestConfig) => {
		try {
			const res = await this.axios_instance.delete<ResponseData<ReturnType>>(`${this.endpoint}${url}`, config);
			throwAuthenticationError(res.data.error);
			return res.data;
		} 
		catch (error) {
