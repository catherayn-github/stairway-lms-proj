...
/* ENTITIES */
import { DecodedJWT, LoginResponse } from "@/app/_entities/interface/auth.interface";
 ...
 */
export const memRefreshToken = pMemoize(async (access_token: string, refresh_token: string) => {
	...
	if (!is_access_token_valid) {
		...
		if (response.result && response.result.new_auth) {
			...
			const decoded_refresh_token: Pick<DecodedJWT, "id" | "exp" | "iat"> = jwtDecode(
				response.result.new_auth.refresh_token,
			);
			return {
	...
	providers: [
		CredentialsProvider({
			...
			async authorize(credentials) {
				...
				const decoded_access_token: DecodedJWT = jwtDecode(response.result.access_token);
				const decoded_refresh_token: Pick<DecodedJWT, "id" | "exp" | "iat"> = jwtDecode(
